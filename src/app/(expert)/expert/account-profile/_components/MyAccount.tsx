...
import { Box, Checkbox, CircularProgress, Link, TextField, Typography } from "@mui/material";
...
const ReactDraftWysiwyg_2 = dynamic(
...
const MyAccount = ({ expert }: { expert?: ExpertCurrent }) => {
  ...
  useEffect(() => {
    ...
    if (expert) {
      setContent(expert.shortDescription);
    }
  ...
  const handleUpdateDescription = async () => {
    ...
    try {
      const res = await PatchExpertUpdateDescription(
      ...
      if (res.status === "success") {
        ...
        enqueueSnackbar("Có lỗi xảy ra khi cập nhật mô tả.", {
          variant: "error",
        });
      }
    } catch (error) {
  ...
  const handleUpdatePrice = async () => {
    ...
    try {
      ...
      if (res.status === "success") {
        ...
        enqueueSnackbar("Có lỗi xảy ra khi cập nhật giá.", {
          variant: "error",
        });
      }
    } catch (error) {
    ...
    <Grid container spacing={2}>
      ...
      <Grid item xs={12}>
        <SubCard title="Giá dịch vụ">
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              ...
              <TextField
                ...
                InputProps={{
                  endAdornment: (
                    <Typography sx={{ ml: -15 }}>/1 buổi phỏng vấn</Typography>
                  ),
                }}
              />
            </Grid>
          ...
          <FlexBox>
            <Link href="https://gotchajob.vn/policy/thanh-toan-cho-chuyen-gia" target="_blank">
              <Typography variant="h5">
                Điều khoản thay đổi giá dịch vụ
              </Typography>
            </Link>
          </FlexBox>
          <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 3 }}>
