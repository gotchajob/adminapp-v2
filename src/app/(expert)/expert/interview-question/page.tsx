...
const interviewQuestions = [
    ...
    questions: [
      {
        id: "mkt-1",
        question:
          "Hãy kể về một thử thách bạn đã đối mặt trong một chiến dịch marketing.",
        type: "Rating",
      },
      {
        id: "mkt-2",
        question:
          "Bạn xây dựng chiến lược marketing cho một sản phẩm mới như thế nào?",
        type: "Text",
      },
      {
        id: "mkt-3",
        question: "Bạn phân bổ ngân sách cho quảng cáo như thế nào?",
        type: "Number",
      },
    ],
  },
  {
    ...
    questions: [
      {
        id: "gd-1",
        question: "Bạn có thể giải thích quy trình thiết kế của bạn không?",
        type: "Text",
      },
      {
        id: "gd-2",
        question: "Bạn sử dụng các công cụ thiết kế nào?",
        type: "Text",
      },
      {
        id: "gd-3",
        question: "Bạn đánh giá hiệu quả thiết kế của mình như thế nào?",
        type: "Rating",
      },
    ],
  },
    ...
    questions: [
      {
        id: "ib-1",
        question:
          "Bạn quản lý mối quan hệ quốc tế với các đối tác như thế nào?",
        type: "Text",
      },
      {
        id: "ib-2",
        question: "Bạn xử lý các rủi ro trong kinh doanh quốc tế như thế nào?",
        type: "Rating",
      },
      {
        id: "ib-3",
        question: "Bạn nghiên cứu thị trường quốc tế như thế nào?",
        type: "Text",
      },
    ],
  },
...
function InterviewQuestionPage() {
  ...
  const renderTable = (questions: any) => (
    <TableContainer
      component={Paper}
      sx={{ mt: 2, boxShadow: "0 3px 5px rgba(0, 0, 0, 0.2)" }}
    >
      <Table>
          ...
          {questions.map((q: any) => {
            ...
            return (
              <TableRow
                key={q.id}
                sx={{
                  backgroundColor: isSelected ? "success.light" : "inherit",
                }}
              >
  ...
  const RenderFeedbackQuestion = (question: FeedbackQuestion) => {
    ...
    if (
      selectedQuestions.find(
        (value) => value.questionId === question.questionId
      )
    ) {
      isSelected = true;
    }
            ...
            <RadioGroup>
              {attitudeOption.map((e, index) => (
                <FormControlLabel
                  ...
                  control={<Radio />}
                  key={index}
                  label={renderLabel(e.option, e.icon)}
                />
              ))}
            </RadioGroup>
          </FormControl>
  ...
  const DialogAddFeedbackQuestion = (
    <Dialog
      open={openAddFeedbackQuestion}
      maxWidth="md"
      fullWidth
      onClose={handleOpenAddFeedbackQuestion}
    >
      <DialogTitle textAlign={"center"}>
        Các câu hỏi đã chọn để phỏng vấn
      </DialogTitle>
      <DialogContent>
        <Box sx={{ p: 1 }}>
          <TreeView
  ...
  return (
    <MainCard>
      ...
      >
        <TreeItem
          nodeId="interviewQuestions"
          label="Câu hỏi buổi phỏng vấn"
          sx={{ color: "#90caf9" }}
        >
          {interviewQuestions.map((category) => (
            <TreeItem
