...
export const RenderTransactionCurrentWithdrawTable = ({
  transactionCurrentWithdraw,
  transactionType,
}: {
  transactionCurrentWithdraw: TransactionCurrentWithdrawRes;
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box
          sx={{ maxWidth: 300, wordWrap: "break-word", whiteSpace: "normal" }}
        >
          {params.value}
        </Box>
      ),
  ...
  const { handleChangeEventText, text, findAllIndexByAnyField } =
    useGetFilter();
      ...
      data = data
        ...
        .filter((row) => {
          ...
          return (
            ...
            formatDate(row.createdAt, "dd/MM/yyyy hh:mm").includes(
              lowerCaseText
            )
          );
        });
