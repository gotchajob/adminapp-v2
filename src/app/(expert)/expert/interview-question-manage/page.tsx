...
import {
  UseGetBookingExpertFeedbackQuestion,
  UseGetBookingExpertFeedbackQuestionCurrent,
} from "hooks/use-get-booking-expert-feedback-question";
import { ExpertToken } from "hooks/use-login";
import { DelBookingExpertFeedbackQuestionById } from "package/api/booking-expert-feedback-question/id";
import { accessToken } from "mapbox-gl";
import { QuestionCategoryById } from "package/api/expert-question-category/id";
import {
  BookingExpertFeedbackQuestion,
  PostBookingExpertFeedbackQuestion,
} from "package/api/booking-expert-feedback-question";
...
function CategoryTable({
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<QuestionCategoryById | null>(null);
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      const res = await PostExpertQuestionCategory(
        {
              ...
              <TableRow key={category.id}>
                <TableCell
                  sx={{
                    ...
                    pointerEvents: categoryStatus[category.id]
                      ? "auto"
                      : "none",
                  }}
                >
...
function InterviewQuestionTable({
  refresh,
  refreshTime,
  token,
}: {
  ...
  const { bookingExpertFeedbackQuestion } =
    UseGetBookingExpertFeedbackQuestionCurrent(token, refreshTime);

  const [selectedQuestion, setSelectedQuestion] = useState<
    BookingExpertFeedbackQuestion | undefined
  >(undefined);
  ...
  const handleAdd = async () => {
    ...
    try {
      if (
        ...
        newQuestion.type === "" ||
        newQuestion.categoryId === 0
      ) {
        return;
      }
      const res = await PostBookingExpertFeedbackQuestion(
        {
  ...
  const handleConfirmDelete = async () => {
    if (selectedQuestion) {
      ...
      try {
        const res = await DelBookingExpertFeedbackQuestionById({
            ...
            <TableBody>
              {bookingExpertFeedbackQuestion?.map((question, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    {
                      expertTypeInputList.find(
                        (value) => value.name === question.type
                      )?.description
                    }
                  </TableCell>
